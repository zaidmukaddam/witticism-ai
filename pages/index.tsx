      ...
      <div className="mx-auto p-4 lg:p-6 max-w-screen-sm min-h-screen">
        <div className="mb-8">
          <div className="mb-2 flex items-center gap-2">
            <Image src="/logo.svg" alt="Logo" width={40} height={37} priority={true} />
            <Link
              href="/"
              className="text-2xl leading-none font-bold tracking-tight hover:underline"
            >
              witticism<span className="text-cyan-500">.</span>ai
            </Link>
          </div>
          <p>
        ...
        {isNotStarted(results) && (
          <div className="flex flex-col gap-12 lg:gap-14">
            ...
            <section>
              ...
              <p>
                <strong>witticism.ai</strong> is an AI joke generation tool built on top of
                OpenAI’s GPT-3 language models. It was fine-tuned on ten thousand late night comedy
                monologue jokes. If you’ve ever wondered what AI would look like if it was trying
                to be funny, this is the place to start.
              </p>
            </section>
          </div>
        )}
        ...
        {isLoading(results) && (
          <div className="animate-pulse">
            ...
            {[...Array(3)].map((_, i) => (
              <div key={i} className="mb-5 p-4 border-2 border-gray-200 rounded-lg">
        ...
        {isLoaded(results) && results.value.status === "error" && (
          <div className="p-5 lg:p-7 mb-4 text-red-700 bg-red-100 rounded-lg" role="alert">
            ...
            witticisms. Please try again shortly.
            <br />
        ...
        {isLoaded(results) && results.value.status === "success" && (
          <div>
            <h2 className="mb-2 text-sm font-bold uppercase tracking-wider">Witticism options:</h2>
            <Typist
            ...
            >
              <div className="flex flex-col gap-4">
                {results.value.results.map((witticism, i, arr) => {
                  const prompt = (results.value as SuggestResponse.Success)?.prompt ?? ""
                  const fullJoke = `${prompt} ${witticism}`.trim()
